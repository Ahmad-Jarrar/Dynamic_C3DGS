...
def readPanopticColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=50):
    cam_infos = []
...
def readPanopticColmapInfo(path, images, eval, multiview=False, duration=50):
    ...
    print("Reading Colmap Scene Info from: ", path)
    reading_dir = "images" if images == None else images
    parentdir = os.path.dirname(path)
    ...
    cam_infos_unsorted = readPanopticColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), near=near, far=far, startime=starttime, duration=duration)
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
    ...
    if eval:
        ...
        train_cam_infos = cam_infos
        test_cam_infos = cam_infos #dummy
...
sceneLoadTypeCallbacks = {
    ...
    "PanopticColmap": readPanopticColmapInfo,
}
